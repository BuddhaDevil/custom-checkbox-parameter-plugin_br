...
function initializationCheckbox(parentDiv,requestBasicUrl,parameterName){
	...
	jQuery.ajax({
				...
				for(i=0;i<result.list.length;i++){
					checkboxDiv.append("<label style='padding:10px 10px 10px 10px;'><input type='checkbox' " + result.list[i].checked + " name='checkbox_" + result.list[i].value  + "'>" + result.list[i].name + "</input></label><br>");
				}
